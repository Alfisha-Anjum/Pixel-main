  ...
  return (
    <div className="bg-gray-50">
      <div className="max-w-7xl mx-auto py-12 px-4 lg:px-8">
        ...
        <div className="mb-10">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Choose a Service Type</h2>
          <ServiceTypeSelector 
            types={[
              { id: "split", name: "Split AC" },
              { id: "window", name: "Window AC" },
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-gray-800">
                Services for {activeType.charAt(0).toUpperCase() + activeType.slice(1)} AC
              </h2>
              {servicesData[activeType as keyof typeof servicesData]?.map((service) => (
                <ServiceListItem
                  key={service.id}
                  service={service}
                  onAdd={handleAddItem}
        ...
        <div className="mt-20 space-y-20 max-w-7xl mx-auto">
          ...
          <section className="relative w-full rounded-xl overflow-hidden min-h-[400px] flex items-center shadow-sm">
            <div className="absolute inset-0 z-0">
               ...
               <Image src={nikImg} alt="Banner" fill className="object-cover opacity-50 grayscale" />
            </div>
            ...
            <div className="relative z-20 w-full px-6 py-12">
              <div className="flex justify-between items-end mb-8 text-white max-w-6xl mx-auto">
                <div>
                  ...
                  <p className="text-gray-300 mt-2">See what our happy customers have to say</p>
                </div>
                <div className="flex gap-4">
                    <button onClick={() => scroll(reviewsRef, 'left')} className="p-2 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm transition">
                        <ChevronLeft className="w-6 h-6" />
                    </button>
                    <button onClick={() => scroll(reviewsRef, 'right')} className="p-2 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm transition">
              ...
              >
                {reviews.map((review) => (
                  <div key={review.id} className="min-w-[350px] md:min-w-[500px] bg-white/10 backdrop-blur-md border border-white/20 p-6 rounded-xl text-white snap-center">
                ...
                <div className="min-w-[200px] flex items-center justify-center">
                    <a href="#" className="text-white hover:text-orange-400 font-semibold flex items-center gap-1">
          ...
          <section className="bg-gray-50">
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">We Covered AC Brand</h2>
                <div className="flex gap-2">
                    <button onClick={() => scroll(brandsRef, 'left')} className="p-2 rounded-full bg-white shadow-sm hover:shadow-md text-gray-600"><ChevronLeft className="w-5 h-5"/></button>
                    <button onClick={() => scroll(brandsRef, 'right')} className="p-2 rounded-full bg-white shadow-sm hover:shadow-md text-gray-600"><ChevronRight className="w-5 h-5"/></button>
                </div>
             </div>
             <div ref={brandsRef} className="flex gap-6 overflow-x-auto pb-4 [&::-webkit-scrollbar]:hidden scroll-smooth">
                {acBrands.map((brand, idx) => (
                    <div key={idx} className="min-w-[180px] h-24 bg-white border border-gray-200 rounded-xl flex items-center justify-center shadow-sm hover:shadow-md transition-shadow">
          ...
          <section className="text-center max-w-4xl mx-auto py-8">
             <h2 className="text-2xl font-bold text-gray-900 mb-4">AC Repair Service in Raipur</h2>
             <p className="text-gray-600 leading-7">
                TasPro offers top-notch AC repair and maintenance services in Raipur. Our team of certified professionals ensures your air conditioner runs efficiently, providing quick cooling and lower power consumption. From gas refilling to circuit repair, we handle it all with precision and care.
             </p>
          </section>

          {/* 4️⃣ Hiring Guide Section */}
          <section className="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
             <h2 className="text-2xl font-bold text-gray-900 mb-4">Hiring guide for AC Repair service in Raipur</h2>
             <div className="prose prose-orange max-w-none text-gray-600">
                <p>
                   When hiring an AC repair service in Raipur, it is crucial to look for verified professionals who offer a warranty on their work. Always check for transparent pricing to avoid hidden costs. A good service provider will inspect your AC thoroughly before suggesting repairs. Ensure that they use genuine spare parts and have a good track record of customer satisfaction. At TasPro, we ensure all these standards are met to give you a hassle-free experience.
                </p>
             </div>
          </section>
          ...
          <section>
             <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">You May Like Our Other Services</h2>
                <div className="flex gap-2">
                    <button onClick={() => scroll(servicesRef, 'left')} className="p-2 rounded-full bg-white shadow-sm hover:shadow-md text-gray-600"><ChevronLeft className="w-5 h-5"/></button>
                    <button onClick={() => scroll(servicesRef, 'right')} className="p-2 rounded-full bg-white shadow-sm hover:shadow-md text-gray-600"><ChevronRight className="w-5 h-5"/></button>
                </div>
             </div>
             
             <div ref={servicesRef} className="flex gap-6 overflow-x-auto pb-6 [&::-webkit-scrollbar]:hidden scroll-smooth -mx-1 px-1">
                {otherServices.map((service) => (
                   <div key={service.id} className="min-w-[260px] bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden group">
                      <div className="h-40 relative bg-gray-100">
                         ...
                         <div className="absolute top-3 left-3 bg-white/90 backdrop-blur px-2 py-1 rounded-md text-xs font-bold flex items-center gap-1">
                            <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" /> {service.rating}
                         </div>
                      </div>
                      <div className="p-4">
                         ...
                         <div className="flex justify-between items-center mt-3">
                            ...
                            <button className="px-4 py-1.5 bg-orange-50 text-orange-600 text-sm font-semibold rounded-lg hover:bg-orange-100 transition-colors">Book Now</button>
                         </div>
                      </div>
                   </div>
                ))}
      ...
      <ServiceModal isOpen={isServiceModalOpen} onClose={() => setIsServiceModalOpen(false)} title={selectedService?.name || "Service Details"}>
        {selectedService && (
            <div className="space-y-4">
                <div className="w-full h-[180px] relative rounded-xl overflow-hidden bg-gray-100 mb-4">
                    <Image src={selectedService.image} alt={selectedService.name} fill className="object-cover" onError={(e) => { e.currentTarget.parentElement.innerHTML = '' }} />
                </div>
      ...
      <DateTimeModal isOpen={isDateTimeModalOpen} onClose={() => setIsDateTimeModalOpen(false)} onContinue={handleDateTimeContinue} />
    </div>
  );
};
