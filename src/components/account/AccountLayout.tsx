    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          <span className="hover:text-orange-500 cursor-pointer" onClick={() => setStep("main")}>Home</span>
          ...
          <div className="flex-1">
            ...
            {step === "main" && (
              <div className="bg-white rounded-[12px] shadow-sm p-6">
                ...
                <div className="grid gap-3">
                  <button onClick={() => setStep("edit")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    Edit Profile
                  </button>
                  <button onClick={() => setStep("address")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    Saved Address
                  </button>
                  <button onClick={() => setStep("wallet")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    My Wallet
                  </button>
                  <button onClick={() => setStep("language")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    Change Language
                  </button>
                  <div className="flex items-center justify-between px-4 py-3 rounded-[12px] border border-gray-100">
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={notification} onChange={() => setNotification((v) => !v)} className="sr-only" />
                      <span className={`w-11 h-6 flex items-center bg-gray-200 rounded-full p-1 ${notification ? 'bg-orange-400' : ''}`}></span>
                    </label>
                  </div>
                  <div className="flex items-center justify-between px-4 py-3 rounded-[12px] border border-gray-100">
                    <span>Dark Mode (Toggle)</span>
                    <label className="inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={dark} onChange={() => setDark((v) => !v)} className="sr-only" />
                      <span className={`w-11 h-6 flex items-center bg-gray-200 rounded-full p-1 ${dark ? 'bg-orange-400' : ''}`}></span>
                    </label>
                  </div>

                  <button onClick={() => setStep("activity")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    My Activity
                  </button>
                  <button onClick={() => setStep("rating")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    My Rating & Reviews
                  </button>
                  <button onClick={() => setStep("coupon")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    My Coupon
                  </button>
                  <button onClick={() => setStep("refer")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    Refer & Earn
                  </button>
                  <button onClick={() => setStep("bank")} className="w-full text-left px-4 py-3 rounded-[12px] border border-gray-100 hover:shadow">
                    Bank Account
                  </button>

                  <div className="pt-4">
                    <button onClick={handleLogout} className="w-full h-11 rounded-[12px] border border-orange-400 text-orange-600">Logout</button>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="bg-white rounded-[16px] shadow-sm p-10">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      <label className="text-xs text-gray-500 font-medium">Select Gender</label>
                      <div className="flex flex-col gap-3">
                        {["I am male", "I am female", "Rather not to say"].map((label) => (
                          <label key={label} className="flex items-center gap-3 cursor-pointer group">
                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${gender === label ? 'border-[#FF6A00]' : 'border-gray-300 group-hover:border-gray-400'}`}>
                              {gender === label && <div className="w-2.5 h-2.5 rounded-full bg-[#FF6A00]" />}
                            </div>
                            <input 
                              type="radio" 
                              name="gender" 
                            ...
                            />
                            <span className={`text-sm ${gender === label ? 'text-black' : 'text-gray-600'}`}>{label}</span>
                          </label>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="mt-8">
                      <button 
                        ...
                        style={{ background: 'linear-gradient(90deg, #FF6A00 0%, #FF8E53 100%)' }}
            ...
            {step === "address" && (
              <div className="bg-white rounded-[12px] shadow-sm p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setAddressListOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-lg">
                      ...
                      <div className="flex justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">{address.label}</span>
                          </div>
                        ...
                        <div className="flex flex-col gap-2">
                          {!address.isDefault && (
                            <button onClick={() => setAddresses(addresses.map(a => ({...a, isDefault: a.id === address.id})))} className="text-orange-600">Set as Default</button>
                          )}
                          <button className="text-blue-600">Edit</button>
                          <button onClick={() => setAddresses(addresses.filter(a => a.id !== address.id))} className="text-red-600">Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="max-w-[390px] mx-auto relative">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="text-[20px] leading-none">{language.flag}</span>
                      <span className="font-medium text-[#333333]">{language.name}</span>
                    </div>
                    <ChevronDown 
                      ...
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-[16px] shadow-[0_20px_40px_rgba(0,0,0,0.12)] py-3 z-20 animate-in fade-in zoom-in-95 duration-200">
                        {languages.map((lang) => (
                          ...
                          >
                            ...
                            <span className="text-sm text-[#444] font-medium">{lang.name}</span>
                            {language.code === lang.code && <Check className="w-4 h-4 text-[#FF7A00] ml-auto" />}
                          </button>
                        ))}
                      </div>
                    </>
            ...
            {step === "refer" && (
              <div className="bg-white rounded-[16px] shadow-sm p-8">
                ...
                <div className="flex flex-col xl:flex-row gap-[60px]">
                  {/* Left Side - Terms */}
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg mb-6">Terms & Conditions</h3>
                    <ol className="list-decimal pl-5 space-y-4 text-[#4B5563] text-[14px] leading-[1.8]">
                      <li>Refer a friend and earn coins when they complete their first service.</li>
                      <li>Coins can be redeemed for discounts on future services.</li>
                      <li>Minimum service value for referral eligibility is ₹500.</li>
                      <li>Referral bonus is credited within 24 hours of service completion.</li>
                      <li>Coins are valid for 1 year from the date of credit.</li>
                      <li>You can refer up to 50 friends per year.</li>
                      <li>Self-referrals are not allowed and will lead to account suspension.</li>
                      <li>TAS Pro reserves the right to modify or end the referral program.</li>
                      <li>Coins cannot be transferred to other accounts or exchanged for cash.</li>
                      <li>Referral code must be applied at the time of booking.</li>
                      <li>Cancelled bookings do not qualify for referral rewards.</li>
                      <li>In case of disputes, TAS Pro decision will be final.</li>
                    </ol>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-10 min-w-[350px] lg:min-w-[390px]">
                    ...
                    <div className="bg-[#0B0F2F] rounded-[24px] p-8 w-full text-center shadow-lg">
                      <h3 className="text-[#FF8A00] font-medium mb-6">Refer a Friend & Earn Coins</h3>
                      ...
                      <div className="flex flex-col items-center mb-8">
                        ...
                        <div className="text-white text-[32px] font-bold leading-tight">{coins}</div>
                      ...
                      <div className="w-full max-w-[280px] aspect-video relative mb-6 bg-gray-50 rounded-lg flex items-center justify-center overflow-hidden">
                         <img 
                           ...
                           onError={(e) => {
                             ...
                             e.currentTarget.parentElement!.innerHTML = '<div class="text-gray-300 text-sm">Illustration</div>';
                           }}
                         />
                      </div>
            ...
            {step === "bank" && (
              <div className="bg-white rounded-[12px] shadow-sm p-6 max-w-2xl">
                ...
                <div className="space-y-3">
                  {banks.length === 0 && <div className="text-gray-500">No bank accounts added.</div>}
                  {banks.map((b) => (
                    <div key={b.id} className="p-3 border rounded-[12px] flex justify-between items-center">
                      <div>
                        <div className="font-medium">{b.bankName}</div>
                        <div className="text-sm text-gray-500">{b.accountNumber}</div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setRemoveConfirm({ open: true, id: b.id })} className="text-red-600">Remove</button>
                      </div>
                    </div>
                  ))}
                  ...
                  <div>
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      ...
                      <div>
                        <Button onClick={() => setBanks([...banks, { id: Date.now().toString(), bankName: 'Demo Bank', accountNumber: 'XXXXXX' }])}>Activate</Button>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {referModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300" onClick={() => setReferModalOpen(false)} />
          <div className="relative bg-white rounded-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-[#0B0F2F] rounded-[20px] p-10 flex flex-col items-center text-center relative overflow-hidden">
              ...
              <button 
                onClick={() => { setReferModalOpen(false); setRedeemOpen(true); }}
              ...
              <button 
                onClick={() => { setReferModalOpen(false); setShareModalOpen(true); }}
      ...
      {redeemOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300" onClick={() => setRedeemOpen(false)} />
          <div className="relative bg-white rounded-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] w-full max-w-sm p-8 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setRedeemOpen(false)} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex justify-between items-center font-bold text-lg">
                ...
                <span className="text-[#FF7A00]">₹{Math.floor(coins / 14)}</span>
              </div>
            </div>

            <button 
              onClick={() => { setRedeemOpen(false); setSuccessOpen(true); }}
      ...
      {shareModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300" onClick={() => setShareModalOpen(false)} />
          <div className="relative bg-white rounded-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] w-full max-w-sm p-8 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-gray-900">Share</h3>
              <button onClick={() => setShareModalOpen(false)} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>

            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl p-3 mb-8">
              <span className="flex-1 text-sm text-gray-600 truncate font-medium">https://taspro.in/ref/john123</span>
            ...
            <div className="grid grid-cols-4 gap-4 mb-8">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs text-gray-600 font-medium">User {i}</span>
                </div>
              ))}
            </div>

            <div className="flex justify-between items-center px-2">
              <button className="w-10 h-10 rounded-xl bg-[#25D366]/10 flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all duration-300 shadow-sm hover:shadow-md"><MessageCircle className="w-5 h-5" /></button>
              <button className="w-10 h-10 rounded-xl bg-[#1877F2]/10 flex items-center justify-center text-[#1877F2] hover:bg-[#1877F2] hover:text-white transition-all duration-300 shadow-sm hover:shadow-md"><Facebook className="w-5 h-5" /></button>
              <button className="w-10 h-10 rounded-xl bg-[#1DA1F2]/10 flex items-center justify-center text-[#1DA1F2] hover:bg-[#1DA1F2] hover:text-white transition-all duration-300 shadow-sm hover:shadow-md"><Twitter className="w-5 h-5" /></button>
              <button className="w-10 h-10 rounded-xl bg-[#E4405F]/10 flex items-center justify-center text-[#E4405F] hover:bg-[#E4405F] hover:text-white transition-all duration-300 shadow-sm hover:shadow-md"><Instagram className="w-5 h-5" /></button>
              <button className="w-10 h-10 rounded-xl bg-[#0A66C2]/10 flex items-center justify-center text-[#0A66C2] hover:bg-[#0A66C2] hover:text-white transition-all duration-300 shadow-sm hover:shadow-md"><Linkedin className="w-5 h-5" /></button>
            </div>
          </div>
        </div>
      )}

      <Modal open={successOpen} onClose={() => setSuccessOpen(false)} title="Success">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600 mt-2">Your coins were successfully redeemed.</p>
          <div className="mt-4">
      ...
      {/* Remove bank confirm */}
      <Modal open={removeConfirm.open} onClose={() => setRemoveConfirm({ open: false })} title="Confirm Remove">
        <div className="space-y-4">
          <p>Are you sure you want to remove this bank?</p>
          <div className="flex gap-2">
            <button onClick={() => setRemoveConfirm({ open: false })} className="h-11 rounded-[12px] px-4 border w-full">Cancel</button>
            <Button onClick={() => handleRemoveBank(removeConfirm.id)}>Confirm</Button>
          </div>
        </div>
      </Modal>
      <Footer />
